class Admin::CruisesController < AdminController
  respond_to :html
  def index
    @all_cruises = Cruise.unscoped.only(:start_date)
    @cruises = Cruise.unscoped.desc(:approved).desc(:start_date,:end_date)
    if params[:year]
  ...
  def show
    @cruise = Cruise.unscoped.where(id: params[:id]).first
