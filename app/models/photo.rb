class Photo < ActiveRecord::Base
  ...
  validates_uniqueness_of :checksum, scope: [:photoable_id], message: 'This photo has already been attached to this report'
  ...
  def remove_image_file
    ...
    if photoable.photos.where(name: name).count <= 1
      FileUtils.remove(file_path) if exists?
    end
  end

  def file_path
    File.join(photoable.export_path, name)
  end

  def url
    File.join(photoable.relative_export_path, name).gsub(/public\//, '')
  end
