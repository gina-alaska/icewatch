  ...
  module ClassMethods
    ...
    def water_coverage_area(observation)
      if observation.ice.total_concentration.nil? or observation.ice.total_concentration == 0
        ...
        melt_pond_percent_coverage(observation.ice_observations.secondary) +
        melt_pond_percent_coverage(observation.ice_observations.tertiary)
        ) / observation.ice.total_concentration
      end
    end
    
    def form_of_water(open_water_code) 
    ...
    def topographic_nature(topo_code)
      case topo_code
      when 200,400
        1
      when 500
    ...
    def partial_without_thickness(observation)
    ...
    def partial_without_surface_coverage(observation)
      partial_concentrations = observation.ice_observations.select(&:partial_concentration)
    ...
    def brash_observation(observation)
      %w{primary secondary tertiary}.each do |type|
        ...
        if has_brash_ice_with_thickness(ice_obs)
          return ice_obs
        end      
      end
      nil
